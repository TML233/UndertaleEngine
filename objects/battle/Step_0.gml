...
if(Flag_Get(FLAG_TYPE.TEMP,FLAG_TEMP.ENCOUNTER) != 9999) {
	...
	if(_state==BATTLE_STATE.MENU){
		...
		if(_menu==BATTLE_MENU.BUTTON){
			...
			if(Input_IsPressed(INPUT.CONFIRM)){
				...
				switch(_menu_choice_button){
					case 0:
						Battle_SetMenu(BATTLE_MENU.FIGHT_TARGET,false,true);
						break;
					case 1:
		...
		if(_menu==BATTLE_MENU.FIGHT_TARGET){
			...
			if(Input_IsPressed(INPUT.UP)){
				if(_menu_choice_enemy>(_enemy[0]!=noone ? 0 : 1)){
					if(_enemy[1]==noone){
						if(_enemy[0]!=noone){
							_menu_choice_enemy=0;
						}
						else {
							_menu_choice_enemy=2;
						}
					}
					else
						_menu_choice_enemy--;
			...
			if(Input_IsPressed(INPUT.CONFIRM)){
				audio_play_sound(snd_menu_confirm,0,false);
				Battle_SetMenu(BATTLE_MENU.FIGHT_AIM,false,true);
			}
		}else
	...
	_menu_choice_button=-9999;
	battle_soul.x=battle_button_fight.x+16;
	battle_soul.y=battle_button_fight.y+22;
	if(Input_IsPressed(INPUT.CONFIRM)) {
		Battle_End();
		for (var i = 0; i < 5; i++;) {
			Music_Stop(i);
